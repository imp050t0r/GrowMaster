(() => {
  ...
  function availableFor(item) {
    const pending = state.queue.reduce((sum, sale) => sum + sale.items.filter(x => x.harvest_id === item.harvest_id).reduce((s, x) => s + x.quantity_kg, 0), 0);
  ...
  function addSelectedQuantity(amount) {
    ...
    else state.cart.push({ harvest_id: item.harvest_id, crop: item.crop, variety: item.variety, quality: item.quality, quantity_kg: quantity, price_per_kg_eur: Number(item.suggested_price_per_kg_eur) });
  ...
  function renderCart() {
    ...
    for (const item of state.cart) {
      ...
      row.innerHTML = `<div><strong>${escapeHtml(item.crop)}${item.variety ? ` · ${escapeHtml(item.variety)}` : ""}</strong><small>${qty(item.quantity_kg)} × ${money(item.price_per_kg_eur)}/kg</small></div><b>${money(lineTotal)}</b>`;
    ...
    try {
      while (state.queue.length && navigator.onLine) {
        ...
        await api("/api/retail-sales", {
          method: "POST",
          body: JSON.stringify({ sale_date: sale.sale_date, payment_method: sale.payment_method, customer_id: null, notes: `POS:${sale.id}`, items: sale.items }),
        });
