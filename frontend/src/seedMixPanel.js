...
export function initSeedMixPanel() {
  ...
  const n = (value, digits = 2) => Number(value || 0).toLocaleString("sl-SI", { maximumFractionDigits: digits });
  ...
  function previewHtml(preview) {
    ...
    const rows = preview.components.map((c) => {
      ...
      const lots = (c.allocations || []).map((a) => `${esc(a.variety || a.lot_number || "lot")}: ${n(a.take_g)} g`).join(" · ");
  ...
  async function preview(form) {
    ...
    const body = { width_m: Number(fd.get("width_m")), length_m: Number(fd.get("length_m")), reserve_pct: Number(fd.get("reserve_pct")), reference: String(fd.get("reference") || "").trim() || null, seed_rates_g_m2: rates };
    state.preview = await apiRequest(`/api/seed-inventory/mix-recipes/${encodeURIComponent(state.selected)}/inventory-preview`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    state.preview._request = body;
  }
  async function consume() {
    if (!state.preview?.inventory_ready || !state.preview?._request) return;
    const result = await apiRequest(`/api/seed-inventory/mix-recipes/${encodeURIComponent(state.selected)}/consume`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(state.preview._request) });
  ...
  function mount(container) {
    ...
    form?.addEventListener("submit", async (event) => { event.preventDefault(); state.error = ""; state.notice = ""; try { await preview(form); mount(container); } catch (error) { state.error = error.message || String(error); mount(container); } });
    container.querySelector("#gm-mix-recipe")?.addEventListener("change", (event) => { state.selected = event.target.value; state.preview = null; mount(container); });
    container.querySelector("#gm-mix-consume")?.addEventListener("click", async () => { if (!window.confirm("Potrdi setev in odštej vse komponente mešanice iz Seed Inventory?")) return; state.error = ""; try { await consume(); mount(container); } catch (error) { state.error = error.message || String(error); mount(container); } });
  }
  return { loadRecipes, mount };
}

const panel = initSeedMixPanel();
const tabs = document.querySelector(".gm-seed-tabs");
const content = document.getElementById("gm-seed-content");
let active = false;

function ensureMixTab() {
  if (!tabs || tabs.querySelector("[data-mix-tab]")) return;
  const button = document.createElement("button");
  button.type = "button";
  button.dataset.mixTab = "1";
  button.textContent = "Mešanice";
  if (active) button.classList.add("active");
  button.addEventListener("click", async (event) => {
    event.stopPropagation();
    active = true;
    tabs.querySelectorAll("button").forEach((b) => b.classList.toggle("active", b === button));
    content.innerHTML = `<div class="gm-seed-card">Nalagam recepture …</div>`;
    try { await panel.loadRecipes(); panel.mount(content); }
    catch (error) { content.innerHTML = `<div class="gm-seed-error">${String(error.message || error)}</div>`; }
  });
...
if (tabs && content) {
  ensureMixTab();
  const observer = new MutationObserver(() => { ensureMixTab(); if (active) { const b = tabs.querySelector("[data-mix-tab]"); tabs.querySelectorAll("button").forEach((x) => x.classList.toggle("active", x === b)); } });
  observer.observe(tabs, { childList: true });
  tabs.addEventListener("click", (event) => { if (event.target.closest("[data-tab]")) active = false; }, true);
}
