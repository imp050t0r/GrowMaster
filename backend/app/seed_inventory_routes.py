...
def seed_inventory_summary(
    crop: str,
    variety: str | None = None,
    unit: str = Query(default="seeds"),
) -> dict:
    try:
...
def plant_requirement(
    target_plants: int = Query(gt=0),
    germination_pct: float = Query(default=95, gt=0, le=100),
    field_emergence_pct: float = Query(default=90, gt=0, le=100),
    ...
    return {
        ...
        "field_emergence_pct": field_emergence_pct,
...
def seed_order_recommendation(
    required_quantity: float = Query(gt=0),
    required_unit: str = Query(default="seeds"),
    available_quantity: float = Query(default=0, ge=0),
    package_size: float | None = Query(default=None, gt=0),
) -> dict:
    try:
