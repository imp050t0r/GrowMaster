...
class MixInventoryRequest(BaseModel):
    width_m: float = Field(gt=0, le=100)
    length_m: float = Field(gt=0, le=1000)
    reserve_pct: float = Field(default=5, ge=0, le=100)
    seed_rates_g_m2: dict[str, float] = Field(
        default_factory=dict,
        description="Setvena norma g/m2 za vsako komponento recepture.",
    )
    reference: str | None = None
...
def mix_bed_requirement(
    ...
    # Legacy read-only endpoint. Missing rates are deliberately reported rather
    # than guessed. The POST preview endpoint accepts the complete agronomic
    # rate map supplied by the client/Plant DB.
...
@router.post("/api/seed-inventory/mix-recipes/{recipe_id}/inventory-preview")
def mix_inventory_preview(recipe_id: str, body: MixInventoryRequest) -> dict:
    """Preview exact lots and shortages before any seed is deducted."""
    try:
        return plan_mix_against_inventory(
            recipe_id,
            body.width_m,
            body.length_m,
            body.seed_rates_g_m2,
            body.reserve_pct,
        )
    except KeyError as exc:
        raise HTTPException(status_code=404, detail="Receptura mešanice ne obstaja.") from exc
...
@router.post("/api/seed-inventory/mix-recipes/{recipe_id}/consume")
def mix_inventory_consume(recipe_id: str, body: MixInventoryRequest) -> dict:
    """Confirm sowing and atomically deduct all recipe components from Seed Inventory."""
    try:
    ...
    return {"message": "Semena za mešanico so knjižena iz Seed Inventory.", **result}
